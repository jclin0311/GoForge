...
import (
	...
	"github.com/goforge/ai-platform/internal/auth"
...
func main() {
	...
	grpcServer := grpc.NewServer(
		grpc.UnaryInterceptor(grpc_middleware.ChainUnaryServer(
			...
			middleware.MetricsUnaryInterceptor("auth-service"),
			rateLimiter.UnaryServerInterceptor(),
		)),
	)

	authServer := auth.NewServer(logger.Log)
	...
	go func() {
		logger.Info("Auth Service listening", zap.Int("port", cfg.GRPC.Port))
