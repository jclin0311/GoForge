...
import (
	...
	"github.com/goforge/ai-platform/internal/model"
...
func main() {
	...
	grpcServer := grpc.NewServer(
		grpc.UnaryInterceptor(grpc_middleware.ChainUnaryServer(
			...
			middleware.MetricsUnaryInterceptor("model-service"),
			rateLimiter.UnaryServerInterceptor(),
		)),
	)

	modelServer := model.NewServer(logger.Log)
	pb.RegisterModelServiceServer(grpcServer, modelServer)

	healthServer := health.NewServer()
	grpc_health_v1.RegisterHealthServer(grpcServer, healthServer)
	healthServer.SetServingStatus("model-service", grpc_health_v1.HealthCheckResponse_SERVING)
	...
	go func() {
		logger.Info("Model Service listening", zap.Int("port", cfg.GRPC.Port))
